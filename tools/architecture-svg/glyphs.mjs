/* The glyph set for the architecture drawing: Lucide, as data.
 *
 * A glyph is the inside of a 24×24 viewBox and nothing more — no width, no
 * stroke, no colour. The renderer wraps it and paints it, the way `lucide-react`
 * does in the web app, so a glyph here and the same glyph in the UI are the
 * same shape. Copied from the library at the version `packages/ui` pins; to
 * change one, replace it with the library's own rather than editing the path.
 */
export const GLYPHS = {
  // Block marks — one per layer.
  ...
  // A data mart's definition type, the badge the plugin leads with.
  ...
  // What a card carries: triggers, relations, and a report's shape.
  'calendar-clock':
  ...
  // The foot: quality, freshness, sharing, owners, last run.
  'shield-check':
  ...
  // The way out to the full list, and the one card with no brand mark.
