...
export class DataStorageApiService extends ApiService {
   ...
   * @param {CreateDataStorageRequestDto} data - The new data to update the storage resource with.
  ...
  async updateDataStorage(
    id: string,
    data: CreateDataStorageRequestDto
  ): Promise<DataStorageResponseDto> {
    return this.patch<DataStorageResponseDto>(`/${id}`, data);
  }
