...
import { type ComponentPropsWithoutRef, type ReactNode, useState, useMemo } from 'react';
...
export function LookerStudioReportCard({
  ...
  const isEnabled = dataMartStatus?.code === DataMartStatus.PUBLISHED;
  const isChecked = !!existingReport;

  // Generate title based on switch state
  const dynamicTitle = isChecked ? 'Available in Looker Studio' : 'Not available in Looker Studio';
