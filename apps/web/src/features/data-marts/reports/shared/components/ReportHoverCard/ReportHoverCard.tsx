...
import { mapReportStatusToStatusType, getReportStatusText } from '../../../../shared/utils';
...
export const ReportHoverCard = React.memo(
  function ReportHoverCard({ report, children }: ReportHoverCardProps) {
    ...
    const reportIcon = useMemo(() => {
      return isGoogleSheetsDestinationConfig(report.destinationConfig) ? (
        <GoogleSheetsIcon size={20} />
      ) : (
        <LookerStudioIcon size={20} />
      );
    ...
    return (
      <HoverCard>
        ...
        <HoverCardContent>
          <HoverCardHeader>
            <HoverCardHeaderIcon>{reportIcon}</HoverCardHeaderIcon>
            <HoverCardHeaderText>
