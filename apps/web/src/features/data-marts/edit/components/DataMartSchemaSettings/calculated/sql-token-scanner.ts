/**
 * A faithful port of the backend's SQL lexer
 * (`apps/backend/src/data-marts/calculated-fields/sql-token-scanner.ts`). Dependency-free by
 * design there, which is exactly what let it move here unchanged — kept byte-for-byte in step so
 * the web editor's idea of "this span is a string/comment/quoted identifier" never disagrees with
 * the backend's. If the backend file changes, port the change here too.
 */
