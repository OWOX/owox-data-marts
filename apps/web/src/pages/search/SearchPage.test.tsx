...
describe('SearchPage', () => {
  ...
  describe('ENTITY_TYPE_META — REPORT', () => {
    ...
    it('renders a REPORT result with its data mart and destination as context', () => {
      ...
      expect(screen.getByText('Orders · Finance Sheets')).toBeInTheDocument();
    });
    ...
    it('falls back to the destination title for a report without a title', () => {
      ...
      expect(within(link).getByText('Finance Sheets')).toBeInTheDocument();
      expect(within(link).getByText('Orders · Finance Sheets')).toBeInTheDocument();
    });
  });
