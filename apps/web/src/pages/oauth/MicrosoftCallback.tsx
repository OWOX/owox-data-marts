...
export function MicrosoftCallback() {
  ...
  useEffect(() => {
    ...
    if (!opener) {
      console.error('MicrosoftCallback: Window opener is missing');
      setStatus('error');
      setErrorMessage(
    ...
    const handleAuthFailure = (errorMsg: string) => {
      ...
      opener.postMessage({ type: 'MICROSOFT_AUTH_ERROR', error: errorMsg }, TARGET_ORIGIN);
    };

    if (error) {
      const errorMsg = errorDescription ?? error;
      console.error('MicrosoftCallback: Auth error received', errorMsg);
    ...
    setStatus('success');
    opener.postMessage({ type: 'MICROSOFT_AUTH_SUCCESS', code, state }, TARGET_ORIGIN);
