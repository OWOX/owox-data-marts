...
import { settingsReducer, fetchSettings, type SettingsState } from './reducers/flags.reducer';
  ...
  return (
    <StoreProvider
      ...
      initialState={{
        ...
        settings: { data: null, callState: RequestStatus.IDLE },
        app: { ready: false },
      }}
    >
      {children}
...
export function AppBootstrap({ children }: { children: React.ReactNode }) {
  ...
  const settingsState = useAppSelector((s: RootState) => s.settings);
  const settingsStatus: SettingsState['callState'] = settingsState.callState;
  ...
  useEffect(() => {
    if (status === AuthStatus.AUTHENTICATED && user?.projectId) {
      ...
      if (settingsStatus === RequestStatus.IDLE) {
        void fetchSettings(dispatch);
      }
    }
  }, [status, user?.projectId, user?.projectTitle, settingsStatus, dispatch]);

  useEffect(() => {
    const ready =
      status === AuthStatus.AUTHENTICATED &&
      Boolean(user?.projectId) &&
      settingsStatus === RequestStatus.LOADED;
    dispatch({ type: APP_SET_READY, payload: ready });
  }, [status, user?.projectId, settingsStatus, dispatch]);

  const loader = useMemo(() => <LoadingSpinner fullScreen message='Loading application...' />, []);

  if (!isReady) {
    if (settingsStatus === RequestStatus.ERROR) {
      return (
        <LoadingSpinner
          fullScreen
          message={`Failed to load app settings: ${settingsState.error ?? ''}`}
        />
      );
    }
    return loader;
