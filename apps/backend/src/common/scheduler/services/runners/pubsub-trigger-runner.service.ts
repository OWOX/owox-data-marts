...
> extends AbstractTriggerRunnerService<T> {
   ...
   *
   * @param message The PubSub message containing the trigger ID
   * @returns A promise that resolves when the message has been processed
   */
  private async handlePubSubMessage(message: Message): Promise<void> {
    try {
      const triggerId = message.data.toString();
      this.logger.log(`Received message with trigger ID: ${triggerId}`);
  ...
  /**
   * Publishes a single trigger's ID to the PubSub topic.
   *
  ...
  private async publishTriggerToPubSub(trigger: T): Promise<string> {
    const dataBuffer = Buffer.from(trigger.id);
    ...
    try {
      ...
      this.logger.debug(`Published message ${messageId} for trigger ${trigger.id}`);
      return messageId;
    } catch (error) {
