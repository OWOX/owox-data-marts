...
export class AppEditionConfig {
  ...
  async actualizeAppEdition(verbose?: boolean): Promise<void> {
    ...
    try {
      ...
      const licenseContext = this.toLicenseContext(
        claims.jti,
        claims.payload as LicenseClaims | undefined
      );
  ...
  private toLicenseContext(jti: string | undefined, claims?: LicenseClaims): LicenseContext {
    ...
    return {
      binding,
      licenseKeyId: jti,
      billingProjectId: claims.billingProjectId,
      expiresAt,
    };
  }
