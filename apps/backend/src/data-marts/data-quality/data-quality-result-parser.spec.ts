...
describe('DataQualityResultParser', () => {
  ...
  it.each([
    ['2026-07-14T00:00:00.000Z', DataQualityCheckStatus.FAILED, 1],
    ['2026-07-15T11:30:00.000Z', DataQualityCheckStatus.PASSED, 0],
    [{ value: '2026-07-15T11:30:00.000Z' }, DataQualityCheckStatus.PASSED, 0],
  ])('parses metadata freshness timestamp %s', async (lastModified, status, violationCount) => {
    const result = await createDataQualityResultParser().parse(
      ...
      executablePlan({
        ...
        thresholdHours: 24,
        queries: [{ purpose: DataQualityQueryPurpose.METADATA_FRESHNESS, sql: 'metadata query' }],
      }),
      [execution(DataQualityQueryPurpose.METADATA_FRESHNESS, [{ last_modified_at: lastModified }])],
  ...
  it('maps unavailable physical metadata to NOT_APPLICABLE', async () => {
    const result = await createDataQualityResultParser().parse(
      DataStorageType.SNOWFLAKE,
      executablePlan({
        category: DataQualityCategory.DATA_FRESHNESS,
      ...
      [execution(DataQualityQueryPurpose.METADATA_FRESHNESS, [])]
    );

    expect(result.status).toBe(DataQualityCheckStatus.NOT_APPLICABLE);
  });

  it.each([
    [DataStorageType.GOOGLE_BIGQUERY, 'NOT_FOUND'],
    [DataStorageType.SNOWFLAKE, 'OBJECT_NOT_FOUND'],
    [DataStorageType.DATABRICKS, 'DELTA_MISSING_DELTA_TABLE'],
  ])(
    'maps provider metadata-unavailable error %s/%s to NOT_APPLICABLE',
    async (storageType, code) => {
      const result = await createDataQualityResultParser().parse(
        storageType,
        executablePlan({
          category: DataQualityCategory.DATA_FRESHNESS,
        ...
        [
          {
            ...
            sql: 'metadata query',
            error: { code, message: 'provider metadata is unavailable', details: null },
          },
        ]
      );
  ...
  it.each([
    [DataStorageType.GOOGLE_BIGQUERY, 'WAREHOUSE_TIMEOUT'],
    [DataStorageType.SNOWFLAKE, 'ACCESS_DENIED'],
    [DataStorageType.DATABRICKS, 'NETWORK_ERROR'],
  ])('keeps metadata infrastructure/query error %s/%s as ERROR', async (storageType, code) => {
    const result = await createDataQualityResultParser().parse(
      storageType,
      executablePlan({
        category: DataQualityCategory.DATA_FRESHNESS,
      ...
      [
        {
          purpose: DataQualityQueryPurpose.METADATA_FRESHNESS,
          sql: 'metadata query',
          error: { code, message: 'warehouse execution failed', details: null },
        },
      ]
    );
  ...
  it('maps an empty Databricks table with no file metadata to NOT_APPLICABLE', async () => {
    const result = await createDataQualityResultParser().parse(
      DataStorageType.DATABRICKS,
      executablePlan({
        category: DataQualityCategory.DATA_FRESHNESS,
        ...
        thresholdHours: 24,
        queries: [
          { purpose: DataQualityQueryPurpose.METADATA_FRESHNESS, sql: 'file metadata query' },
        ],
      }),
      [execution(DataQualityQueryPurpose.METADATA_FRESHNESS, [{ last_modified_at: null }])]
    );
