...
const ExternalCredentialManifestSchema = z.object({
  ...
  delivery: z.object({ type: z.literal('credential-definition') }),
  credential: z.object({
    name: z
      .string()
      .trim()
      .min(1)
      .max(64)
      .refine(name => JS_IDENTIFIER.test(name), 'credential.name must be a JavaScript identifier')
    ...
    authentication: z.object({
      ...
      placement: z.object({
        ...
        name: z
          ...
          .refine(name => !BLOCKED_AUTH_HEADERS.has(name.toLowerCase()), 'header is reserved'),
...
export function parseExternalCredentialManifest(
  ...
  if (!parsed.success) {
    return { ok: false, detail: parsed.error.issues[0]?.message ?? 'manifest is invalid' };
  }
