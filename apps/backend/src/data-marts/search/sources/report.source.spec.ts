...
describe('ReportIndexableSource', () => {
  ...
  async function seedReportOnMart(martOverrides: Partial<DataMart> = {}): Promise<Report> {
  ...
  describe('access predicate parity with data mart visibility', () => {
    ...
    it('viewer sees a report of a data mart shared for reporting', async () => {
    ...
    it('viewer does not see a report of a private data mart', async () => {
      const report = await seedReportOnMart({
    ...
    it('admin sees every report and bypasses roleScope resolution', async () => {
      const report = await seedReportOnMart({
        availableForReporting: false,
        availableForMaintenance: false,
      });
    ...
    it('hides a shared report from a context-scoped user without an overlapping context', async () => {
