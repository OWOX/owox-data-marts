...
describe('AgentFlowPromptBuilder', () => {
  it('builds initial messages with conversation turns and focused user prompt', () => {
    ...
    expect(initialMessages).toHaveLength(6);
    ...
    expect(initialMessages[3]).toEqual({
      role: AiRole.ASSISTANT,
      content: 'Hi! How can I help?',
    });
    expect(initialMessages[4]).toEqual({
      role: AiRole.USER,
      content: 'show me monthly consumption',
    });
    expect(initialMessages[5].role).toBe(AiRole.USER);
    ...
    expect(initialMessages[5].content).toContain('Latest user message to handle:');
    expect(initialMessages[5].content).toContain('show me monthly consumption');
    expect(initialMessages[5].content).not.toContain('Recent turns:');
    expect(initialMessages[5].content).toContain(
