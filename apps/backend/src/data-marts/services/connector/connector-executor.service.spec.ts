...
import { ProjectBilling, RunKind } from '../project-billing/project-billing';
...
describe('ConnectorExecutorService', () => {
  ...
  it('handles balance check failure', async () => {
    ...
    (projectBilling.authorizeRun as jest.Mock).mockRejectedValue(
  ...
  it('registers consumption when abort arrives after a successful connector upload', async () => {
    ...
    expect(projectBilling.registerConsumption).toHaveBeenCalledWith(expect.anything(), {
      ...
      dataMart: expect.objectContaining({ id: 'dm-1' }),
