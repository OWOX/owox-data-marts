...
function buildService(
  ...
  return new InternalProjectBilling(configService, connectorService);
}
...
const grant = { projectId: 'proj-1', runKind: RunKind.HTTP_DATA_RUN };

describe('InternalProjectBilling', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  describe('authorizeRun', () => {
    it('grants without calling the Balance API when unconfigured', async () => {
      const service = buildService({});

      await expect(
        service.authorizeRun({ projectId: 'proj-1', runKind: RunKind.HTTP_DATA_RUN })
      ).resolves.toEqual({ projectId: 'proj-1', runKind: RunKind.HTTP_DATA_RUN });
      expect(fetchWithBackoffMock).not.toHaveBeenCalled();
    });

    it('grants when the Balance API allows the operation', async () => {
      fetchWithBackoffMock.mockResolvedValue(
        balanceResponse({ allowed: true, blockedReasons: [] })
      );
      const service = buildService(BALANCE_ENV);

      const result = await service.authorizeRun({
        projectId: 'proj-1',
        runKind: RunKind.SHEETS_REPORT_RUN,
      });

      expect(result).toEqual({ projectId: 'proj-1', runKind: RunKind.SHEETS_REPORT_RUN });
    ...
    it('propagates a Balance API transport failure', async () => {
      ...
      await expect(
        service.authorizeRun({ projectId: 'proj-1', runKind: RunKind.HTTP_DATA_RUN })
      ).rejects.toThrow('balance unreachable');
    });
    ...
    it('propagates a non-ok Balance API response', async () => {
      ...
      await expect(
        service.authorizeRun({ projectId: 'proj-1', runKind: RunKind.HTTP_DATA_RUN })
      ).rejects.toThrow('Balance API request failed with status 503');
    });
  });
  ...
  describe('registerConsumption', () => {
    it('publishes an HTTP Data run to its topic with the run id as reportRunId', async () => {
      ...
      await service.registerConsumption(grant, {
    ...
    it('publishes a Looker report run to the Looker topic', async () => {
      ...
      await service.registerConsumption(grant, {
        ...
        report: fakeReport(DataDestinationType.LOOKER_STUDIO),
      });
    ...
    it.each([
      ...
      await service.registerConsumption(grant, {
        ...
        report: fakeReport(destinationType),
      });
    ...
    it('swallows an unsupported email-based destination type without publishing', async () => {
      ...
      await expect(
        service.registerConsumption(grant, {
          kind: RunKind.EMAIL_BASED_REPORT_RUN,
          report: fakeReport(DataDestinationType.GOOGLE_SHEETS),
        })
