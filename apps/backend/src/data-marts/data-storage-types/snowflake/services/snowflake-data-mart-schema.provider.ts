...
export class SnowflakeDataMartSchemaProvider implements DataMartSchemaProvider {
  ...
  async getActualDataMartSchema(
    ...
    // Build the query with LIMIT 1 to get schema (need at least 1 row to get column names)
    const baseQuery = this.queryBuilder.buildQuery(definition);
    const query = `${baseQuery} LIMIT 0`;
    const adapter = this.adapterFactory.create(credentials, config);

    try {
      // Execute query to get result with column metadata
  ...
  private createFieldFromResult(columnName: string, type: string): SnowflakeDataMartSchema['fields'][0] {
    if (!columnName) {
