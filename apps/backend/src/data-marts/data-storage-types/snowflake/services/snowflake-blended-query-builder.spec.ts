...
describe('SnowflakeBlendedQueryBuilder', () => {
  ...
  describe('single subsidiary with LISTAGG', () => {
    it('generates correct SQL with pre-aggregation and LEFT JOIN using LISTAGG', () => {
      ...
      const sql = builder.buildBlendedQuery(
        buildContext('mydb."myschema"."customers"', [chain], ['customer_name', 'order_names'])
      );

      expect(sql.trimStart().startsWith('WITH')).toBe(true);
      expect(sql).toContain('main AS (');
      expect(sql).toContain('SELECT * FROM mydb."myschema"."customers"');
      expect(sql).toContain('orders_raw AS (');
      expect(sql).toContain('SELECT * FROM mydb."myschema"."orders"');
    ...
    it('does not use STRING_AGG syntax', () => {
      ...
      const sql = builder.buildBlendedQuery(
        buildContext('mydb."myschema"."customers"', [chain], ['order_names'])
      );
  ...
  describe('multi-key join', () => {
    it('generates AND in ON clause and multi-column GROUP BY for multiple join keys', () => {
      ...
      const sql = builder.buildBlendedQuery(
        buildContext('mydb."myschema"."customers"', [chain], ['event_names'])
      );
  ...
  describe('non-STRING_AGG aggregation', () => {
    it('uses COUNT function correctly', () => {
      ...
      const sql = builder.buildBlendedQuery(
        buildContext('mydb."myschema"."customers"', [chain], ['order_count'])
      );
