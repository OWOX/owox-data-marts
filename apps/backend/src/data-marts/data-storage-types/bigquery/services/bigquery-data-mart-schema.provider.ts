...
export class BigQueryDataMartSchemaProvider implements DataMartSchemaProvider {
  ...
  async getActualDataMartSchema(
    ...
    const schema = await this.getNativeSchema(dataMartDefinition, config, credentials);
  ...
  private async getNativeSchema(
    ...
    return dryRunResult.schema;
  }

  private parseFields(fields: TableSchema['fields']): BigqueryDataMartSchema['fields'] {
    if (!fields) {
      return [];
    }
    ...
    return fields.map(field => {
      ...
      if (!parsedFieldMode) {
        this.logger.error(
          `Failed to parse field mode: ${field.mode}, defaulting to ${BigQueryFieldMode.NULLABLE}`
        );
