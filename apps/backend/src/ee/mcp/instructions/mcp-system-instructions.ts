...
- update_report returns the report's resulting fields and output controls and, by default, re-runs a push-destination report whose export changed (run.status="queued"); a name-only or message-only change does not run unless run_immediately=true. Send only the controls that change — anything omitted is kept.
