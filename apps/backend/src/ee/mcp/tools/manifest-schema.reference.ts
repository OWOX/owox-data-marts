 ...
 */
export const MANIFEST_SCHEMA_VERSION = '2026-07-24';
...
Note: this poll \`backoff\` is a completely separate mechanism from a node's \`errorHandler.backoff\` (§15) — the former paces the async job-status loop, the latter paces HTTP-error retries.
...
Optional, node-level, and applies to **sync nodes only** (an async node paces itself through \`poll.backoff\`, §12, instead). Matches an HTTP error response against an ordered list of filters; the first matching filter decides what happens:
