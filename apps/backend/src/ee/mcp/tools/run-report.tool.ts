...
export class RunReportTool implements McpToolDefinition<RunReportInput> {
  ...
  readonly description =
    'Start a report run by report_id, delivering fresh data to its push destination (Google Sheets, Email, Slack, Microsoft Teams, or Google Chat). Use this when the user wants up-to-date data in an existing report. Returns immediately with just report_id and run_id — it does not wait for the run to finish and does not report status. Report runs routinely take several minutes: after starting one, poll get_report_run_status with the returned ids until it reports "success" or "failed", following the guidance in its message field (wait up to 15 seconds between checks if you can wait between tool calls; if you cannot, check immediately rather than ending your reply). A "running" status on those checks is normal — do not stop polling on your own initiative or report a failure just because the run is still running (stop early only if the user asks). Each run_report call starts a new, separately billed run, so never call run_report again for the same report while a run is in progress — keep polling the existing run_id instead. If the report already has a run in flight, this call fails with "Report is already running or pending"; that is not a report failure. Pull-based consumers (Data Studio, HTTP Data API) fetch data themselves and cannot be run with this tool. Billed as a standard Report Run for the destination, not as an MCP query.';
