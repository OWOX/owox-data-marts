 ...
 * mounted only on the Cloud deployment, which can impersonate the OWOX license service account.
 */
@Module({})
export class LicenseKeysModule {
  static register(): DynamicModule {
    const cloudEnabled = process.env.LICENSE_ISSUANCE_ENABLED === 'true';
    // Surfaces the License Keys tab through the /api/flags whitelist.
    process.env.LICENSE_KEYS_MANAGEMENT_ENABLED = String(cloudEnabled);
