...
| `IDP_OWOX_EXTENSION_MICROSOFT_ENABLED`               |    No    |                   `false`                   | Enables the Microsoft NAA extension routes                                                                    |
| `IDP_OWOX_EXTENSION_MICROSOFT_AUDIENCES`             |  Cond.   |                      –                      | Exact comma-separated OWOX API audiences; required when NAA is enabled                                        |
| `IDP_OWOX_EXTENSION_MICROSOFT_SCOPE`                 |  Cond.   |                      –                      | Exact delegated scope from the Entra access-token `scp`; required when NAA is enabled                         |
| `IDP_OWOX_EXTENSION_ALLOWED_ORIGINS`                 |  Cond.   |                      –                      | Exact comma-separated add-in origins; required when NAA is enabled                                            |
| `IDP_OWOX_EXTENSION_MICROSOFT_JWKS_URL`              |    No    |      Microsoft common v2 JWKS endpoint      | Entra signing-key endpoint                                                                                    |
| `IDP_OWOX_EXTENSION_MICROSOFT_ISSUER_AUTHORITY`      |    No    |     `https://login.microsoftonline.com`     | Authority used for exact `iss`/`tid` consistency                                                              |
| `IDP_OWOX_EXTENSION_ASSERTION_RATE_LIMIT_PER_MINUTE` |    No    |                    `30`                     | Per-process, per-client-IP assertion exchange limit                                                           |
| `IDP_OWOX_EXTENSION_CLOCK_TOLERANCE`                 |    No    |                    `5s`                     | Entra assertion timestamp clock tolerance                                                                     |
