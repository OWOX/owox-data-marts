...
describe('account-resolver', () => {
  ...
  it('returns provider-specific account when found', async () => {
    ...
    const store = createStoreMock({
      getAccountByUserIdAndProvider: jest.fn().mockResolvedValue(account),
    });
  ...
  it('falls back to latest account when preferred is missing', async () => {
    ...
    const store = createStoreMock({
      getAccountByUserIdAndProvider: jest.fn().mockResolvedValue(null),
      getAccountByUserId: jest.fn().mockResolvedValue(fallback),
    });
