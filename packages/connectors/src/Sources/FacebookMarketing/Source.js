...
var FacebookMarketingSource = class FacebookMarketingSource extends AbstractSource {
  ...
  constructor(config) {
    super(config.mergeParameters({
      ...
      ProcessShortLinks: {
        ...
        description: "Resolve short links to their landing page: link_url_asset.parsed_url on the Ad Insights by Link URL Asset endpoint, and link_url_parsed / object_url_parsed on Ad Creatives",
  ...
  */
  async fetchData(nodeName, accountId, fields, startDate = null, onPageFetched = null) {
    ...
    // Connector-only short link columns are filled after the fetch; never request them from Graph
  ...
  async _fetchInsightsData({ nodeName, accountId, fields, timeRange, url }) {
    ...
    return await this._fetchPaginatedData(requestUrl, nodeName, fields);
  }
