...
- **Plugin page.** Publishers see a "Release issues" card listing each rejected release with its code, reason, and the time of the check.

The collection-compatibility safeguard is also back, now gated on SemVer: a minor or patch release may add collections and change action mappings, but cannot remove a collection or change its name, scope, or entity binding — the previous version stays current and the reason says how to proceed. A major version bump waives the check, so a breaking collection change ships by declaring itself as one.
