...
Because a change can disconnect fields that reports and relationships rely on, it is confirmed in a dialog that states what is preserved, what may break, and how many relationships and reports depend on this Data Mart. A published Data Mart stays published. Switching to a table, view or pattern checks the new target against the storage first, so an unreachable source is refused instead of being saved and failing later.
