...
- A selection that includes a metric gets the **first aggregation its Data Mart governance allows**, in a fixed priority order (`SUM` → `AVG` → `MIN` → `MAX` for numbers, `MIN` → `MAX` for dates and times). A text or boolean metric is left alone — nothing its governance allows returns the column you selected — and so is a report that selects a column from a joined Data Mart.
