...
A table with no recorded data changes in the last year shows **Unknown** with a note. A dropped-and-recreated table answers for its current generation only, never for its predecessor's history. Materialized views and other objects the history cannot answer for appear as unknown sources and the coverage becomes partial, so the reported value stays a lower bound: the real time can only be more recent.
