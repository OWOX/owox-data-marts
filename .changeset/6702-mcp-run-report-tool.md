...
AI assistants connected to OWOX over MCP can now run a report by its id: `run_report` starts the run and returns immediately with just the report id and run id, while data is delivered to the report's push destination (Google Sheets, Email, Slack, Microsoft Teams, or Google Chat). `get_report_run_status` then reports the current outcome for that run — success, failed with an error message, or still running — along with `queued_at`, nullable `started_at`, and the backend `raw_status`.
