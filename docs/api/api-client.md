...
titles of its Data Mart and destination. A report is visible when its Data Mart is visible. A
...
| `is_blank`, `is_not_blank`, `is_true`, `is_false`                                                                   | No `value`                                                              | `{ column: 'Country', operator: 'is_blank' }`                                         |
...
`is_blank` / `is_not_blank` match by what a rendered cell shows: on string columns a value is blank when it is `NULL`, `''`, or whitespace-only; on every other type it is blank only when `NULL`. The pre-merge operators `is_empty`, `is_not_empty`, `is_null`, and `is_not_null` are still accepted for saved configs and older callers, but new rules should use the blank pair.

The negative operators `neq`, `not_contains`, `not_regex`, and `not_in` are NULL-inclusive: they keep rows where the column is `NULL`, treating a missing value as "not equal to". Add a separate `is_not_blank` rule on the same column to also drop missing values.
