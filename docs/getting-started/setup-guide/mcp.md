...
| `joined_fields`         | Fields contributed by blended/joined data marts (empty when the data mart has no joins), each with its qualified `<alias>__<field>` name, source data mart, type, `category`, and effective `allowedAggregations`                   |
...
Use this tool when you need to understand the fields available in a specific data mart — both its native fields and any joined fields you can then query with `query_data_mart`. A field's `allowedAggregations` and its category's entry in `operators_by_category` tell the assistant exactly which aggregations and filter operators that field supports, so queries can be built without trial and error. It does not return sample values, data freshness, owners, or actual data rows. To learn how joined/blended fields are set up, see [Joinable Data Marts](./joinable-data-marts.md).
