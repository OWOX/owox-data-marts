...
| `post_aggregation_filters` | Present only when the report has post-aggregation rules created in the OWOX UI; they cannot be set through the assistant, and replacing `filters` removes them              |
...
Use this tool before creating a report, to find one that already exports what you asked for, and before scheduling or changing a report's cadence, to see what already exists. A filter created in the OWOX UI with a preset the assistant cannot express (for example "today") is returned as stored; the assistant keeps such a rule by leaving `filters` out of an update.
...
| `shared_with_requester`   | Google Sheets only, new files only. `false` if the sheet could not be shared with you — opening the link may require requesting access  |
