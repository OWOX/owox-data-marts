...
| `filters`      | Post-join filters on the blended result. Row-level predicates applied to raw values before any aggregation — there is no `HAVING`, so they cannot threshold an aggregated total. Which operators a field accepts depends on its type — see `operators_by_category` from `get_data_mart_details_by_id`; several filters combine with AND |
