...
- **Google Sheets**: a new Google Sheet is created automatically and linked to the report, then the initial run writes data into it by default; both the file and its sheet are named after the report. Unlike every other tool, this path reaches outside OWOX: it creates a file in Google Drive and attempts to share it with you. When you ask for several related exports, the assistant passes the first report's `spreadsheet_id` to the next `add_report` calls, so each report becomes a separate sheet of the same document instead of a separate file; that sheet is named after its report too. See [Google Sheets destination](../../destinations/supported-destinations/google-sheets.md) for how to set the destination up.
