...
Coverage is best effort per storage. Google BigQuery, AWS Redshift, and AWS Athena are supported, and OWOX resolves views and SQL data marts through to their underlying base tables. Sharded and wildcard table sets collapse into one entry. Other storages currently report `unavailable`. `sources` deliberately omits views: a view's own modification time reflects a change to its definition, not to any data. Redshift metadata can lag real writes by up to ~5 minutes, and older Redshift releases do not report modification times at all; Athena is exact for Iceberg tables, while Hive tables report `null` with a note (their catalog stores no data-change time) — see [Data Last Updated](./data-last-updated.md) for storage-specific caveats.
