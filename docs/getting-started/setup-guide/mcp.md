...
By default, a push-destination report is run again after an update that changed what it exports, so the Google Sheet (or the next message) reflects the new definition; a name-only or message-only change does not trigger a run. The response returns the report as it is after the update.
...
| `run_immediately` | Optional. Omit for the default (run a push-destination report when the export changed; no run for a name-only or message-only change). `false` updates without delivering; `true` also re-delivers after a name or message change. Looker Studio is pull-based: `true` is rejected |
