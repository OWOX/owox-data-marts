...
A report is returned when its data mart is visible to your [project role](../../project/roles-and-permissions.md). Use `get_data_mart_reports` when the data mart is already known and you need every report with its full definition.
...
- **Google Sheets**: a new Google Sheet is created automatically and linked to the report, then the initial run writes data into it by default. Unlike every other tool, this path reaches outside OWOX: it creates a file in Google Drive and attempts to share it with you. When you ask for several related exports, the assistant passes the first report's `spreadsheet_id` to the next `add_report` calls, so each report becomes a separate sheet of the same document instead of a separate file; the sheet is named after the report. See [Google Sheets destination](../../destinations/supported-destinations/google-sheets.md) for how to set the destination up.
