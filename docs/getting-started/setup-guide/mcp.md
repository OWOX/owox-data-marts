...
| `fields`                   | Column names the report exports, or `["*"]` for every field                                                                                                    |
...
| `aggregations`             | Aggregations applied on every run                                                                                                                              |
...
By default, a Google Sheets report is run again after an update that changed what it exports, so the sheet reflects the new definition; a name-only or message-only change does not trigger a run. Email, Slack, Microsoft Teams, and Google Chat reports are not re-sent by an update — a run would deliver the message to every recipient or channel — unless you explicitly ask to send it now. The response returns the report as it is after the update.
