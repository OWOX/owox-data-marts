...
Coverage is best effort per storage. Google BigQuery, AWS Redshift, and Snowflake are supported. OWOX resolves views and SQL data marts through to their underlying base tables. Sharded and wildcard table sets collapse into one entry. Other storages currently report `unavailable`. `sources` deliberately omits views: a view's own modification time reflects a change to its definition, not to any data. Redshift metadata can lag real writes by up to ~5 minutes. Older Redshift releases do not report modification times at all. Snowflake reports the start of the hour of the last data change, and the value can trail reality by several hours. See [Data Last Updated](./data-last-updated.md) for storage-specific caveats.
