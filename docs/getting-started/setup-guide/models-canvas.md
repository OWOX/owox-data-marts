...
- The number of **triggers**, **reports** built on the Data Mart, and **relationships**. These badges share a line while they fit the card. The relationships count covers the whole storage, whatever the filters hide. It counts the relationships the Data Mart defines and the ones that point to it.
...
A count of zero shows no badge, and a line left without badges is dropped. The sharing icons appear once the canvas has loaded the Data Mart details.
