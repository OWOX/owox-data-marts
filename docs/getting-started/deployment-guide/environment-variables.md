...
Without `PLUGIN_HOST_PUBLIC_ORIGIN`, a vendor who correctly names your origin in a
`Content-Security-Policy: frame-ancestors` directive is still rejected, because the server
has no way to know what that origin is.
